...
if(isset($_POST['submit'])){
	...
	if (!empty($_FILES['profilepic']['name'])) {
		...
		$file = $_FILES['profilepic']; //Get filename from form 'files' function, creates an array
		$fileName = $_FILES['profilepic']['name']; //Get name in file array
		$fileTmpName = $_FILES['profilepic']['tmp_name']; //Get temporary name in file array (it gets a new name while its uploading)
		$fileSize = $_FILES['profilepic']['size']; //get size in file array
		$fileError = $_FILES['profilepic']['error']; //get error report in array
		$fileType = $_FILES['profilepic']['type']; //get type in array
		...
		$sql = "UPDATE member SET picture = '$fileNameNew' WHERE username = '$whatUser'"; //Updates filename in the member table
